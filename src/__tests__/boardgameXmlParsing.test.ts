...
describe('XML Parsing Tests', () => {
  ...
  beforeAll(() => {
    ...
    const fixturePath = path.join(__dirname, './fixtures/stevmachine_collection.xml');
  ...
  describe('boardgamegeek collection XML parsing', () => {
    ...
    it('should parse XML structure correctly', async () => {
      ...
      expect(Object.keys(result)).toEqual(['totalitems', 'termsofuse', 'pubdate', 'item']);
      
      // Check metadata
      expect(result.totalitems).toBe('302');
      expect(result.pubdate).toBe('Mon, 29 Sep 2025 23:09:34 +0000');
      expect(result.termsofuse).toBe('https://boardgamegeek.com/xmlapi/termsofuse');
    });
    ...
    it('should parse first item correctly', async () => {
      ...
      expect(Object.keys(firstItem)).toEqual([
        'objecttype', 'objectid', 'subtype', 'collid', 'name', 
        'yearpublished', 'image', 'thumbnail', 'stats', 'status', 'numplays'
      ]);
    ...
    it('should handle different item types correctly', async () => {
      ...
      const subtypes = [...new Set(result.item.map((item: any) => item.subtype))];
      expect(subtypes.length).toBeGreaterThan(0);
      
      // Check that all items have valid subtypes
      subtypes.forEach(subtype => {
        expect(['boardgame', 'boardgameexpansion', 'rpgitem', 'videogame', 'boardgameaccessory']).toContain(subtype);
      });
    });
  });
});
