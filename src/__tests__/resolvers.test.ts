...
describe('GraphQL Resolvers', () => {
  ...
  describe('Query Resolvers', () => {
    ...
    describe('things', () => {
      it('should call bggAPI.getThings with correct ids', async () => {
        ...
        mockBGGDataSource.getThings.mockResolvedValue(mockThings);

        const result = await resolvers.Query!.things!(null, { ids: ['13', '926'] }, mockContext);
    ...
    describe('search', () => {
      it('should call bggAPI.searchThings with correct parameters', async () => {
        ...
        mockBGGDataSource.searchThings.mockResolvedValue(mockResults);
        ...
        expect(mockBGGDataSource.searchThings).toHaveBeenCalledWith('Catan', 'BOARDGAME', false);
        expect(result).toEqual(mockResults);
      });

      it('should handle optional parameters', async () => {
        const mockResults = [{ id: '13', name: 'CATAN', isExpansion: false }];
        ...
        expect(mockBGGDataSource.searchThings).toHaveBeenCalledWith('Catan', undefined, undefined);
        expect(result).toEqual(mockResults);
      });
    });
    ...
    describe('user', () => {
      it('should call bggAPI.getUser with correct username', async () => {
        ...
        mockBGGDataSource.getUser.mockResolvedValue(mockUser);

        const result = await resolvers.Query!.user!(null, { username: 'testuser' }, mockContext);
    ...
    describe('userCollection', () => {
      it('should call bggAPI.getUserCollection with correct parameters', async () => {
        ...
        mockBGGDataSource.getUserCollection.mockResolvedValue(mockCollection);

        const result = await resolvers.Query!.userCollection!(
          null,
    ...
    describe('userPlays', () => {
      it('should call bggAPI.getUserPlays and format response correctly', async () => {
        ...
        mockBGGDataSource.getUserPlays.mockResolvedValue(mockPlays);

        const result = await resolvers.Query!.userPlays!(
          null,
    ...
    describe('hotItems', () => {
      it('should call bggAPI.getHotItems with correct type', async () => {
        ...
        mockBGGDataSource.getHotItems.mockResolvedValue(mockHotItems);

        const result = await resolvers.Query!.hotItems!(null, { type: 'BOARDGAME' }, mockContext);
  ...
  describe('Thing Resolvers', () => {
    describe('baseGame', () => {
      it('should return null for non-expansion items', async () => {
        ...
        const result = await resolvers.Thing!.baseGame!(mockThing, {}, mockContext);
      ...
      it('should fetch base game for expansion with base game link', async () => {
        ...
        mockBGGDataSource.getThing.mockResolvedValue(mockBaseGame);
    ...
    describe('expansionFor', () => {
      it('should return empty array for expansion items', async () => {
        ...
        const result = await resolvers.Thing!.expansionFor!(mockThing, {}, mockContext);
      ...
      it('should fetch expansions for base game with expansion links', async () => {
        ...
        mockBGGDataSource.getThings.mockResolvedValue(mockExpansions);
      ...
      it('should limit to 10 expansions and batch in groups of 3', async () => {
        ...
        mockBGGDataSource.getThings
          .mockResolvedValueOnce(mockExpansions.slice(0, 3))
          .mockResolvedValueOnce(mockExpansions.slice(3, 6))
          .mockResolvedValueOnce(mockExpansions.slice(6, 9))
          .mockResolvedValueOnce(mockExpansions.slice(9, 10));
  ...
  describe('Error Handling', () => {
    it('should handle dataSource errors in Query resolvers', async () => {
      ...
      await expect(
        resolvers.Query!.thing!(null, { id: '13' }, mockContext)
  ...
  describe('Edge Cases', () => {
    it('should handle null/undefined parent objects', async () => {
      const result = await resolvers.Thing!.baseGame!(null, {}, mockContext);
      expect(result).toBeNull();
    });
  ...
  describe('Performance', () => {
    it('should not make unnecessary API calls', async () => {
      ...
      await resolvers.Thing!.expansionFor!(mockThing, {}, mockContext);
    ...
    it('should batch API calls efficiently', async () => {
      ...
      mockBGGDataSource.getThings.mockResolvedValue([]);
